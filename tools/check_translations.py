...
SOURCE_DIR = os.path.join(os.path.dirname(__file__), '..', 'storefront.koplugin')
MASTER_LANG = 'en'

# Narrow allowlist of keys that legitimately don't need translations (symbols, paths, brand names, etc.)
ALLOWLIST = {
    'storefront_title',        # "Storefront" (brand name)
...
def check_translations():
    print("--- Checking Storefront Translation Sync Status ---")
        ...
        for file in files:
            if file.endswith('.lua'):
                with open(os.path.join(root, file), 'r', encoding='utf-8', errors='ignore') as f:
                    content = f.read()
                    # Find loc:t("key") or _("key") or Localization:t("key")
                    matches = re.finditer(r'(?:loc:t|_|Localization:t)\(\s*[\"\']([^\"\']*)[\"\']', content)
                    for m in matches:
                        used_keys.add(m.group(1))
                    # Find fallbacks in localization_storefront.lua
                    if 'localization_storefront.lua' in file:
                        fb_matches = re.finditer(r'([a-zA-Z0-9_]+)\s*=\s*\"', content)
                        for m in fb_matches:
                            used_keys.add(m.group(1))

    # Remove non-keys or structural tokens if any matched
    used_keys.discard("")
    ...
    if not os.path.exists(LANGUAGES_DIR):
        print(f"Error: Languages directory {LANGUAGES_DIR} not found!")
        sys.exit(1)
